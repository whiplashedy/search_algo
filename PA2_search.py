...
def UCSearch(start, goal, barrier):
    ...
    while len(fringe) > 0:
        ...
        temp = get_neighbors(curNode)
        for item in temp:
            gCost = move_cost(item, barrier)
            fCost = heuristic(item, goal) + gCost
            if item not in G:
                parent[item] = curNode
            if curNode in G :
                G[item] = gCost + G[curNode]
            else:
                G[item] = gCost
            fringe.add(item)
        ############################################################################################
...
def aStarSearch(start, goal, barrier):
    ...
    while len(fringe) > 0:
        ...
        # for item in temp:
        #     temp2 = G[curNode] + move_cost(item, barrier) + heuristic(item, goal)
        #     if item not in G:
        #         parent[item] = curNode
        #     G[item] = G[curNode] + move_cost(item, barrier)
        #     F[item] = temp2
    
            
            # if item not in fringe:
            #     fringe.add(item)
